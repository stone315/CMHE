...
import os
import ast  # to safely parse string list

from Contextual_Learning import CMDP, estimate_performance_gp, equation4_acquisition
import wandb
import logging
...
Sys_para = np.array([0.031, 0.000023951, 0.000023951, 0.000032347]) # updated by the author of NeuroBEM, used for evaluation
...
def Train(epsilon0, gmin0, device='cpu', config=None):
    with wandb.init(project='ContextNeuralMHE_implementation', config=config):
        config = wandb.config
        ...
        model_QR.to(device)  # Move model to the specified device (CPU or GPU)
        ...
        #train_set = {'a': "processed_data/merged_2021-02-18-13-44-23_seg_2.csv"} # a 3d circle trajectory with the maximum velocity of 9.6930 m/s
        train_set = {35: "processed_data/high_wind_tailwind/traj_0/logged_vars.pkl", 10 : "processed_data/low_wind_tailwind/traj_0/logged_vars.pkl",
        } # a figure-8 trajectory with the maximum velocity of 5 m/s and wind disturbance
        # sort train_set by key
        train_set = dict(sorted(train_set.items()))  # sort by key to ensure consistent order
        keytovalue = {0 : 0, 10: 1.3, 35: 4.2} # wind speed in m/s
        ...
        K = 3
        k = 1
        rand_initialization = False  # If True, the first context is chosen randomly; if False, the first context is the middle of the list
        trained_contexts = []
        rewards = []

        # context is the values in keytovalue
        context = list(keytovalue.values())
        Loss_list = np.zeros((K, len(context)))  # Store the loss for each context in each iteration
        Best_Loss = {x: -float('inf') for x in context}  # Store the best loss for each context
        Best_list = np.zeros((K, len(context)))  # Store the best loss for each context in each iteration
        gp_est_list = np.zeros((K, len(context)))
        ...
        while k <= K:
            ...
            if trained_contexts:
                X_train = np.array(trained_contexts).reshape(-1, 1)
                Y_train = np.array(rewards)
                X_query = np.array(context).reshape(-1, 1)
                acquisition_values = []

                # Store Y_train in the gp_est_list for this iteration based on each element of X_train in index of context list
                for i, x in enumerate(X_train):
                    idx = context.index(x)
                    gp_est_list[k-1][idx] = Y_train[i]

                # Estimate the performance using Gaussian Process
                mu, sigma = estimate_performance_gp(X_train, Y_train, X_query)

                # Store mu to the gp_est_list for this iteration based on each element of X_query in index of context list
                for i, x in enumerate(X_query):
                    idx = context.index(x)
                    gp_est_list[k-1][idx] = mu[i]


                for i in range(len(context)):
                    
                    a_x = equation4_acquisition(
                        mu[i], sigma[i], beta= 1,
                        context=context[i],
                        X_all=context,
                        prev_best=Best_Loss,
                        slope= 0.5 # Adjust slope based on your problem
                    )
                    acquisition_values.append(a_x)

                # choose the context with the maximum acquisition value but no in the trained_contexts
                acquisition_values = np.array(acquisition_values)
                acquisition_values[np.isin(context, trained_contexts)] = -np.inf
                xk_idx = np.argmax(acquisition_values)
            else:
                # Two case: one is random and the other is the medium in the list
                # If no trained contexts, select the first context randomly
                if rand_initialization:
                    xk_idx = np.random.choice(len(context))  # Random first choice
                else:
                    xk_idx = len(context) // 2

            # get the key   in keytovalue where the value is context[xk_idx]
            xk = context[xk_idx]
            keys = [list(keytovalue.keys())[list(keytovalue.values()).index(xk)]]
            trained_contexts.append(xk)  # Add the selected context to trained contexts
            print(f"Selected context: {xk} (key: {keys})")

            

            delta_cost = delta_cost_0
            n_ep = 0

            # reset the model
            model_QR = torch.load(PATH0)  # load the initial model
            model_QR.to(device)  # Move model to the specified device (CPU or GPU)
            ...
            # for k in range(1):
                ...
                for key in keys:
                    it = 0
                    Trained_episode += [n_ep_global]
                    # Obtain the size of the data
                    dataset    = pd.read_pickle(train_set[key])
                    min_len = min(len(v) for v in dataset.values())
                    dataset_trimmed = {k: v[:min_len] for k, v in dataset.items()}
                    dataframe  = pd.DataFrame(dataset_trimmed)
                    # Obtain the sequences of the state, and acceleration from the dataframe
                    
                    angaccx_seq, angaccy_seq, angaccz_seq = dataframe['angular_acceleration_x'], dataframe['angular_acceleration_y'], dataframe['angular_acceleration_z']
                    angvelx_seq, angvely_seq, angvelz_seq = dataframe['gyro_x'], dataframe['gyro_y'], dataframe['gyro_z']
                    qx_seq, qy_seq, qz_seq, qw_seq = dataframe['qx'], dataframe['qy'], dataframe['qz'], dataframe['qw']
                    accx_seq, accy_seq, accz_seq = dataframe['ax'], dataframe['ay'], dataframe['az']
                    velx_seq, vely_seq, velz_seq = dataframe['vx'], dataframe['vy'], dataframe['vz']
                    ...
                    flag = 0
                    vx, vy, vz = [],[],[]
                    for j in range(n_start,n_start+ min_len - 1): # n_start + 4000: these data points covers a limited range of velocity from 0.05 to 5 m/s for the circle trajectory; 
                        ...
                        measurement = torch.tensor(measurement, dtype=torch.float32).to(device)  # Move to the specified device
                        ...
                        # Assuming weight_para is a 2D array: shape (1, 25)
                        weight_values = weight_para.flatten()  # shape: (25,)

                        # Optional: define names for clarity (match your SetPara order)
                        param_names = [
                        *["P0_" + str(i) for i in range(12)],
                        "gamma_r",
                        *["R_" + str(i) for i in range(1, 6)],  # r11 is fixed, so start from R_1
                        "gamma_q",
                        *["Q_" + str(i) for i in range(6)]
                        ]

                        # Create a dictionary to log
                        log_dict = {f"weight_para/{xk}/{name}": val for name, val in zip(param_names, weight_values)}
                        ...
                        if n_ep> 0: # start learning at the second episode. The first episode is set to be unlearned for comparison
                            ...
                            measurement = torch.tensor(measurement, dtype=torch.float32).to(device)  # Move to the specified device
                            dldp = torch.tensor(dldp, dtype=torch.float32).to(device)  # Move to the specified device
                ...
                wandb.log({f"train/{xk}/mean_loss": mean_loss})
                Mean_Loss += [mean_loss]
                Trained += [n_ep_global]
                ...
                if n_ep >=300: # stop learning as it is too time-consuming
                    break
                '''
                if epsilon > 1:
                    break
                ...
                if not os.path.exists("trained_data_implementation"):
                    os.makedirs("trained_data_implementation")
                np.save('trained_data_implementation_tail/Mean_loss', Mean_Loss)
                PATH1 = "trained_data_implementation_tail/Trained_model.pt"
                torch.save(model_QR, PATH1)
                np.save(f'trained_data_implementation_tail/Trained episodes{xk}', Trained_episode)
                np.save(f'trained_data_implementation_tail/epsilon_trained{xk}', epsilon)
                np.save(f'trained_data_implementation_tail/gmin_trained{xk}', gmin)
                


            # store loss k, context xk_idx
            Loss_list[k-1, xk_idx] = mean_loss
            Best_Loss[xk] = -mean_loss
            rewards.append(-mean_loss) 

            # compute the test loss for the current model
            # all key in train_set.keys() except the current key
            for key in train_set.keys():
                ...
                for j in range(n_start,n_start+min_len - 1): # n_start + 4000: these data points covers a limited range of velocity from 0.05 to 5 m/s for the circle trajectory; 
                    ...
                    tunable_para = convert(model_QR(measurement))
                    P0, gamma_r, gamma_q, R_N, Q_N1, weight_para = SetPara(r11, epsilon, gmin, tunable_para)
                    #print('Testing=', n_ep, 'sample=', it, 'p1=', P0[0, 0], 'gamma1=', gamma_r,'gamma2=', gamma_q,'r1=', R_N[0, 0], 'r2=', R_N[1, 1],'r3=', R_N[2, 2])
                    #print('Testing=', n_ep, 'sample=', it, 'q1=', Q_N1[0, 0], 'q2=', Q_N1[1, 1], 'q3=', Q_N1[2, 2])
                    ...
                    df_t, dt_t = GroundTruth(w_B, acc, uav.m, uav.J)
                    df_t       = np.matmul(R_B, df_t) # transformed to the world frame
                    #print('Testing=', n_ep, 'sample=', it, 'Dis_x=', df_t[0, 0], 'df_Imh_x=', df_Imh[0, 0], 'Dis_y=', df_t[1, 0], 'df_Imh_y=', df_Imh[1, 0], 'Dis_z=', df_t[2, 0],
                    #        'df_Imh_z=', df_Imh[2, 0],'norm of v_I=',LA.norm(v_I))
                ...
                # Loss_list store key index
                xk_idx = list(train_set.keys()).index(key)
                Loss_list[k-1, xk_idx] = mean_loss

                if Best_Loss[keytovalue[key]] < -mean_loss:
                    Best_Loss[keytovalue[key]] = -mean_loss
                Best_list[k-1, xk_idx] = Best_Loss[keytovalue[key]]
        ...
        for step, loss in enumerate(Mean_Loss):
            wandb.log({"Mean Loss": loss}, step=step)
            logging.info("Mean Loss at step {}: {}".format(step, loss))

        
        for k in range(K):
            for step, loss in enumerate(Loss_list[k,:]):
                wandb.log({"Loss for K="+str(k+1): loss}, step=step)
                logging.info("Loss for K={} at step {}: {}".format(k+1, step, loss))
        
        for k in range(K):
            for step, best in enumerate(Best_list[k,:]):
                wandb.log({"Best Loss for K="+str(k+1): best}, step=step)
                logging.info("Best Loss for K={} at step {}: {}".format(k+1, step, best))

        for k in range(K):
            for step, est in enumerate(gp_est_list[k,:]):
                wandb.log({"GP Estimate for K="+str(k+1): est}, step=step)
                logging.info("GP Estimate for K={} at step {}: {}".format(k+1, step, est))
        
        for step, context in enumerate(trained_contexts):
            wandb.log({"Trained Context": context}, step=step)
            logging.info("Trained Context at step {}: {}".format(step, context))

        plt.figure(1)
        plt.plot(Trained, Mean_Loss, linewidth=1.5,marker='o')
        plt.title('Mean loss in training episodes')
        plt.xlabel('Number of episodes')
        plt.ylabel('Mean loss')
        plt.grid()
        plt.savefig('plots_testset/trained_data_implementation/' + str(seed) + 'mean_loss_train_reproduction.png',dpi=600)


        # figure2 plot Loss list as a line chart that each line is K, x is the value in keytovalue
        plt.figure(2)
        for k in range(K):
            plot_x = [keytovalue[key] for key in train_set.keys()]
            plot_y = Loss_list[k,:]
            plt.plot(plot_x, plot_y, linewidth=1.5, marker='o', label='K='+str(k+1)+', policy ='+str(trained_contexts[k]))

        plt.legend()
        plt.title('Mean loss in testing set with different policy')
        plt.xlabel('Velocity space')
        plt.ylabel('Mean loss')
        plt.grid()
        plt.savefig('plots_testset/trained_data_implementation/' + str(seed) + 'mean_loss_test_reproduction.png',dpi=600)

        #  plot gp_est_list
        plt.figure(3)
        for k in range(K):
            plot_x = [keytovalue[key] for key in train_set.keys()]
            plot_y = gp_est_list[k,:]
            plt.plot(plot_x, plot_y, linewidth=1.5, marker='o', label='K='+str(k+1)+', policy ='+str(trained_contexts[k]))
        plt.xlabel('Velocity space')
        plt.ylabel('GP estimate')
        plt.title('GP estimate in testing set')
        #set legend value is by k
        plt.legend()
        plt.grid()
        plt.savefig('plots_testset/trained_data_implementation/' + str(seed) + 'gp_estimate_test_reproduction.png',dpi=600)

        # plot trained context in order: x axis is k, y axis is the trained context
        plt.figure(4)
        plt.plot(trained_contexts, linewidth=1.5, marker='o')
        plt.title('Trained context')
        plt.xlabel('Context index')
        plt.ylabel('Trained context value in time series')
        plt.grid()
        plt.savefig('plots_testset/trained_data_implementation/' + str(seed) + 'trained_context_reproduction.png',dpi=600)
        ...
        plt.title('Best loss in testing set')
        plt.legend()
        plt.grid()
        plt.savefig('plots_testset/trained_data_implementation/' + str(seed) + 'best_loss_test_reproduction.png',dpi=600)
...
def Evaluate():
    ...
    # compute RMSE
    # body frame
    rmse_fx = format(mean_squared_error(fx_mhe, Gt_fx),'.3f')
    rmse_fy = format(mean_squared_error(fy_mhe, Gt_fy),'.3f')
    rmse_fz = format(mean_squared_error(fz_mhe, Gt_fz),'.3f')
    rmse_tx = format(mean_squared_error(tx_mhe, Gt_tx),'.3f')
    rmse_ty = format(mean_squared_error(ty_mhe, Gt_ty),'.3f')
    rmse_tz = format(mean_squared_error(tz_mhe, Gt_tz),'.3f')
    rmse_fx_bemnn  = format(mean_squared_error(Bemnn_fx, Gt_fx),'.3f')
    rmse_fy_bemnn  = format(mean_squared_error(Bemnn_fy, Gt_fy),'.3f')
    rmse_fz_bemnn  = format(mean_squared_error(Bemnn_fz, Gt_fz),'.3f')
    rmse_tx_bemnn  = format(mean_squared_error(Bemnn_tx, Gt_tx),'.3f')
    rmse_ty_bemnn  = format(mean_squared_error(Bemnn_ty, Gt_ty),'.3f')
    rmse_tz_bemnn  = format(mean_squared_error(Bemnn_tz, Gt_tz),'.3f')
    # scalar-based error
    rmse_fxy       = format(mean_squared_error(fxy_mhe, Gt_fxy),'.3f')
    rmse_txy       = format(mean_squared_error(txy_mhe, Gt_txy),'.3f')
    rmse_f         = format(mean_squared_error(f_mhe, Gt_f),'.3f')
    rmse_t         = format(mean_squared_error(t_mhe, Gt_t),'.3f')
    rmse_fxy_bemnn = format(mean_squared_error(Bemnn_fxy, Gt_fxy),'.3f')
    rmse_txy_bemnn = format(mean_squared_error(Bemnn_txy, Gt_txy),'.3f')
    rmse_f_bemnn   = format(mean_squared_error(Bemnn_f, Gt_f),'.3f')
    rmse_t_bemnn   = format(mean_squared_error(Bemnn_t, Gt_t),'.3f')
    # vector-based error
    rmse_fxy_v     = format(mean_squared_error(E_MHE_fxy, E_des_fxy),'.3f')
    rmse_f_v       = format(mean_squared_error(E_MHE_f, E_des_f),'.3f')
    rmse_txy_v     = format(mean_squared_error(E_MHE_txy, E_des_txy),'.3f')
    rmse_t_v       = format(mean_squared_error(E_MHE_t, E_des_t),'.3f')
    rmse_fxy_v_bem = format(mean_squared_error(E_BEM_fxy, E_des_fxy),'.3f')
    rmse_f_v_bem   = format(mean_squared_error(E_BEM_f, E_des_f),'.3f')
    rmse_txy_v_bem = format(mean_squared_error(E_BEM_txy, E_des_txy),'.3f')
    rmse_t_v_bem   = format(mean_squared_error(E_BEM_t, E_des_t),'.3f')
    ...
    # Disturbance force
    plt.figure(2)
    plt.plot(list(Time), list(Gt_fxy), linewidth=1.5, linestyle='--', alpha=0.5)
    ...
    # plt.show()
    plt.figure(3)
    plt.plot(Time, Gt_fz, linewidth=1.5, linestyle='--', alpha=0.5)
    ...
    # Disturbance torque
    plt.figure(4)
    plt.plot(Time, Gt_txy, linewidth=1, linestyle='--', alpha=0.5)
    ...
    # plt.show()
    plt.figure(5)
    plt.plot(Time, Gt_tz, linewidth=1, linestyle='--', alpha=0.5)
